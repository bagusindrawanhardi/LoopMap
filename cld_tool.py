...
def parse_dominant_years(dominant_period: str) -> tuple[int | None, int | None]:
    """Return (start_year, end_year) parsed from a dominant_period string."""
    text = re.sub(r"\s*\(.*?\)", "", dominant_period).strip()
    m = re.search(r"(\d{4})\s*[–\-]\s*(\d{4}|present)", text, re.IGNORECASE)
    if m:
        start = int(m.group(1))
        end = date.today().year + 4 if m.group(2).lower() == "present" else int(m.group(2))
        return start, end
...
def render_map_node(
    ...
    tip_data = {"kind": "node", "label": label, "loops": loops or []}
    tip_attr = f' data-tip="{html.escape(json.dumps(tip_data, ensure_ascii=False))}"'
    lines = svg_text(label, 18)
    start_y = y - ((len(lines) - 1) * line_height / 2)
    text = "".join(
        f'<text x="{x:.1f}" y="{start_y + i * line_height:.1f}" class="map-variable-label">{html.escape(line)}</text>'
        for i, line in enumerate(lines)
    )
    return f'<g class="map-node variable-node"{node_attr}{tip_attr}>{text}</g>'
...
def render_map_edge(
    ...
    # Offset the path endpoints to clear node text labels (~28 px).
    ...
    pol_key = {"positive": "pos", "+": "pos", "plus": "pos",
               "negative": "neg", "-": "neg", "minus": "neg"}.get(normalize_name(edge.polarity), "unk")
    marker_id = f"map-arrow-inf-{pol_key}" if edge.inferred_from_loop else f"map-arrow-{pol_key}"
    marker = f"url(#{marker_id})"
    color = polarity_color(edge.polarity)
    tip_data = {
        "kind": "edge",
        "src": edge.source,
        "tgt": edge.target,
    ...
    return (
        ...
        f'fill="none" stroke="{color}" stroke-width="{stroke_w}"{dash} marker-end="{marker}" data-marker="{marker_id}" />'
        f'<text x="{label_x:.1f}" y="{label_y:.1f}" class="map-polarity" fill="{color}">{polarity_symbol(edge.polarity)}</text>'
        "</g>"
    )
...
def write_system_map_html(
    ...
    node_pos_data = {
        slugify(n): {"x": round(px, 1), "y": round(py, 1)}
    ...
    for loop_data in payload["loops"]:  # type: ignore[index]
        ...
        if lid and lid in loop_data_by_id:
            ...
            dp = str(loop_data.get("dominant_period", ""))  # type: ignore[union-attr]
            loop_data_by_id[lid]["dominant_period"] = dp
            ys, ye = parse_dominant_years(dp)
            loop_data_by_id[lid]["year_start"] = ys
            loop_data_by_id[lid]["year_end"] = ye
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .loop-btn.loop-fading {{
      opacity: 0.38;
      filter: grayscale(60%);
    }}
    .loop-btn.loop-inactive {{
      opacity: 0.18;
    ...
    body.dark .timeline-inline {{ border-color: #1e3358; }}
    body.dark .timeline-year {{ color: #60a5fa; }}
    body.dark #timeline-slider {{ accent-color: #3b82f6; }}
...
<body>
  <div class="toolbar">
    <div class="toolbar-row1">
      ...
      <div class="timeline-inline" id="timeline-bar">
        <span class="loop-bar-label">Timeline</span>
        <input type="range" id="timeline-slider" oninput="onTimelineChange(this.value)" />
        <span class="timeline-year" id="timeline-year-label"></span>
        <button class="timeline-reset" onclick="resetTimeline()" title="Show all periods">All</button>
      </div>
    </div>
    <div class="loop-bar">
    ...
    <div class="canvas-wrap">
      ...
      <div class="canvas" id="canvas">
        <svg id="main-svg" role="img" aria-label="Causal loop system map">
          <defs>
            <marker id="map-arrow-pos" markerWidth="16" markerHeight="16" refX="14" refY="8" orient="auto">
              <path d="M2,2 L14,8 L2,14 Z" fill="#15803d"></path>
            </marker>
            <marker id="map-arrow-neg" markerWidth="16" markerHeight="16" refX="14" refY="8" orient="auto">
              <path d="M2,2 L14,8 L2,14 Z" fill="#dc2626"></path>
            </marker>
            <marker id="map-arrow-unk" markerWidth="16" markerHeight="16" refX="14" refY="8" orient="auto">
              <path d="M2,2 L14,8 L2,14 Z" fill="#6b7280"></path>
            </marker>
            <marker id="map-arrow-inf-pos" markerWidth="14" markerHeight="14" refX="12" refY="7" orient="auto">
              <path d="M2,2 L12,7 L2,12 Z" fill="#15803d"></path>
            </marker>
            <marker id="map-arrow-inf-neg" markerWidth="14" markerHeight="14" refX="12" refY="7" orient="auto">
              <path d="M2,2 L12,7 L2,12 Z" fill="#dc2626"></path>
            </marker>
            <marker id="map-arrow-inf-unk" markerWidth="14" markerHeight="14" refX="12" refY="7" orient="auto">
              <path d="M2,2 L12,7 L2,12 Z" fill="#6b7280"></path>
            </marker>
  ...
  <script>
    ...
    /* ── Timeline / dominance slider ───────────────────────────────── */
    (function() {{
      var loops = Object.values(LOOP_DATA);
      var years = loops.map(function(l) {{ return [l.year_start, l.year_end]; }})
                       .flat().filter(function(y) {{ return y !== null && y !== undefined; }});
      if (!years.length) {{
    ...
    function applyLoopDominance(year) {{
      timelineActive = true;

      /* 1. Compute each loop's temporal state */
      var loopState = {{}};
      Object.values(LOOP_DATA).forEach(function(loop) {{
        var ys = loop.year_start, ye = loop.year_end;
        var state;
        if (ys == null || ye == null) {{
          state = 'active';
        }} else if (year >= ys && year <= ye) {{
          state = 'active';
        }} else if (year >= ys - 1 && year <= ye + 1) {{
          state = 'fading';
        }} else {{
          state = 'inactive';
        }}
        loopState[loop.id] = state;

        /* dim / restore loop buttons */
        var btn = document.querySelector('[data-loop="' + loop.id + '"]');
        if (btn) {{
          if (state === 'inactive') {{
            btn.style.opacity = '0.15'; btn.style.filter = 'grayscale(1)'; btn.style.pointerEvents = 'none';
          }} else if (state === 'fading') {{
            btn.style.opacity = '0.4';  btn.style.filter = 'grayscale(0.5)'; btn.style.pointerEvents = '';
          }} else {{
            btn.style.opacity = '1';    btn.style.filter = ''; btn.style.pointerEvents = '';
          }}
        }}
      }});

      /* best state among a list of loop IDs */
      function bestState(loopIds) {{
        var best = 'inactive';
        (loopIds || []).forEach(function(lid) {{
          var s = loopState[lid] || 'inactive';
          if (s === 'active') best = 'active';
          else if (s === 'fading' && best === 'inactive') best = 'fading';
        }});
        return best;
      }}

      /* 2. Apply to SVG nodes */
      document.querySelectorAll('.map-node').forEach(function(el) {{
        var raw = el.getAttribute('data-tip');
        var loops = [];
        if (raw) {{ try {{ loops = JSON.parse(raw).loops || []; }} catch(_) {{}} }}
        var s = loops.length ? bestState(loops) : 'active';
        el.style.opacity = s === 'inactive' ? '0.08' : s === 'fading' ? '0.38' : '1';
      }});

      /* 3. Apply to SVG edges — loop IDs are in data-loops (space-separated) */
      document.querySelectorAll('.map-edge').forEach(function(el) {{
        var raw = el.getAttribute('data-loops') || '';
        var loops = raw.trim() ? raw.trim().split(' ').filter(function(s){{return s;}}) : [];
        var s = loops.length ? bestState(loops) : 'active';
        var isInf = el.classList.contains('inferred');
        if (s === 'inactive') {{
          el.style.opacity = '0.05';
        }} else if (s === 'fading') {{
          el.style.opacity = isInf ? '0.18' : '0.32';
        }} else {{
          el.style.opacity = isInf ? (inferredShowing ? '0.45' : '0') : '1';
        }}
      }});
    }}
    ...
    function resetTimeline() {{
      ...
      document.querySelectorAll('[data-loop]').forEach(function(b) {{
        b.style.opacity = ''; b.style.filter = ''; b.style.pointerEvents = '';
      }});
      document.querySelectorAll('.map-node').forEach(function(el) {{ el.style.opacity = ''; }});
      document.querySelectorAll('.map-edge').forEach(function(el) {{
        var isInf = el.classList.contains('inferred');
        el.style.opacity = isInf ? (inferredShowing ? '0.45' : '0') : '1';
      }});
    ...
    function clearHighlight(resetButtons) {{
      ...
      document.querySelectorAll('.map-edge').forEach(function(el) {{
        ...
        if (p) {{
          p.style.strokeWidth = '';
          const orig = p.getAttribute('data-marker');
          p.setAttribute('marker-end', orig ? 'url(#' + orig + ')' : 'url(#map-arrow-unk)');
        }}
      }});
    ...
    (function() {{
      ...
      function buildReport() {{
        ...
        if (VARIABLE_DATA && VARIABLE_DATA.length) {{
          ...
          VARIABLE_DATA.forEach(function(v) {{
            html += '<div class="rpt-var">'
              + '<div class="rpt-var-label">' + esc(v.label) + '</div>'
              + '<div class="rpt-var-type">' + (v.unit ? esc(v.unit) : '') + '</div>'
      ...
      window.submitVariable = function() {{
        ...
        fetch('/api/variable', {{
          method: 'POST',
          headers: {{'Content-Type': 'application/json'}},
          body: JSON.stringify({{name: name, unit: unit, definition: def}})
        }}).then(function(r) {{
          if (!r.ok) {{ r.text().then(function(t) {{ alert('Error: ' + t); }}); return; }}
          closeVarDlg();
          var slug    = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
          var tipData = JSON.stringify({{kind:'node', label: name, loops:[]}});
          ...
          nodePosMap[slug]          = {{x: cx, y: cy}};
          NODE_POSITIONS_DATA[slug] = {{x: cx, y: cy}};
          var svgNS = 'http://www.w3.org/2000/svg';
          var g = document.createElementNS(svgNS, 'g');
          g.setAttribute('class', 'map-node variable-node');
          ...
          txt.setAttribute('class', 'map-variable-label');
